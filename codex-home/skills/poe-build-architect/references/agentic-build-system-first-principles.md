...
- Product behavior proof requires spawned product-agent authorship. If the
  engineering/orchestrator thread chooses the PoE build changes, writes the
  Direct Build decision ledger, edits that ledger after product-agent return, or
  uses a spawned product agent only as an end-of-run verifier, the attempt is
  development/regression evidence only and must not be accepted as product-agent
  behavior.
...
- the product-agent behavioral proof ledger was authored and returned by the
  spawned product PoE Architect using the installed skill, not by the
  engineering/orchestrator thread or a script fixture;
