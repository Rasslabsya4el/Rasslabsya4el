...
- Tested hypothesis (`composition_summary.tested_hypothesis`): `{{ composition_summary.tested_hypothesis }}`
- Build archetype (`composition_summary.build_archetype`): `{{ composition_summary.build_archetype }}`
...
- Ready PoB import (`composition_summary.ready_pob_import`):
  ...
  - Import code locator (`composition_summary.ready_pob_import.import_code_locator`)
...
## 6. Cost/Value Summary

- Mode (`cost_value_summary.mode`): `{{ cost_value_summary.mode }}`
- Status (`cost_value_summary.status`): `{{ cost_value_summary.status }}`
- Selected decisions (`cost_value_summary.selected_decisions[]`):
  - Row ID (`cost_value_summary.selected_decisions[].row_id`)
  - Mutation kind (`cost_value_summary.selected_decisions[].mutation_kind`)
  - Selected decision (`cost_value_summary.selected_decisions[].selected_decision`)
  - Rejected alternatives short (`cost_value_summary.selected_decisions[].rejected_alternatives_short[]`)
  - Resource cost (`cost_value_summary.selected_decisions[].resource_cost`)
  - Measured/expected value (`cost_value_summary.selected_decisions[].measured_or_expected_value`)
  - Impact categories (`cost_value_summary.selected_decisions[].impact_categories[]`)
  - Uncertainty (`cost_value_summary.selected_decisions[].uncertainty`)
  - Missing evidence (`cost_value_summary.selected_decisions[].missing_evidence[]`)
  - Artifact refs (`cost_value_summary.selected_decisions[].artifact_refs`)
- Aggregate artifact refs (`cost_value_summary.artifact_refs`): `{{ cost_value_summary.artifact_refs }}`
- Missing evidence (`cost_value_summary.missing_evidence[]`): `{{ cost_value_summary.missing_evidence[] }}`
- Notes (`cost_value_summary.notes[]`): `{{ cost_value_summary.notes[] }}`
